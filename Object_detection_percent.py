...
# =========================================================================
# python Object_detection_percent.py > eye_bridge_v3/f_thick.csv
# 폴더 내 이미지 불러오기"
folder_path = "eye_bridge_v3\\f_vague\\"
# folder_path = "G:\\Face_image\\main_original_real\\side_nose_filter\\"
folder_list = os.listdir(folder_path)

# 결과 출력해서 확인하려면 output 경로 수정
output_dir = "eye_bridge_v3\\f_vague_output\\"
# output_dir = "F:\\head_cut_output\\"
# 모델, 라벨맵 이름 설정
model = 'eye_bridge_v3.pb'
label = 'eye_bridge_v3.pbtxt'
...
for item in folder_list:  # 폴더의 파일이름 얻기
    ...
    if (label_str != ""):
        ...
        # 라벨 찾은 개수 세기
        count = count + 1
        detect_dir = output_dir + 'detect_'
        # if not os.path.exists(detect_dir):
        #     os.mkdir(detect_dir)
        if label_score < 50:
            sub_dir = detect_dir + "10to50\\"
        elif label_score >=50 and label_score < 70 :
            sub_dir = detect_dir + "50to70\\"
        elif label_score >=70 and label_score < 90 :
            sub_dir = detect_dir + "70to90\\"
        elif label_score >=90 :
            sub_dir = detect_dir + "upper90\\"

        if not os.path.exists(sub_dir):
            os.mkdir(sub_dir)

        cv2.imwrite(sub_dir + item, image)
        # print(sub_dir+item)
    else:
